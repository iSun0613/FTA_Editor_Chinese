...
8. **计算正确性修复与逻辑门扩展**：修正原版概率计算中"小于 0.000001 一律抹零"的问题，小概率事件（如 1e-6 量级的事故概率）不再被算成 0；逻辑门在与门（AND）/ 或门（OR）之外补齐 **异或门（XOR）、非门（NOT）、表决门（VOTER，可设 k/n 阈值，缺省为多数表决）**，均按标准可靠性公式计算；节点编辑对话框可直接选择门类型、设置表决阈值。
9. **界面响应优化**：图形渲染与 AI 请求改在后台线程执行，大树渲染或 AI 分析时界面不再卡死。
10. **界面全面改版为 macOS 苹果风格**：主窗口与全部弹窗（节点编辑、AI 设置、AI 更改建议、图形渲染窗口）统一为白色卡片式面板 + 扁平控件：输入框扁平化、点击时亮起蓝色聚焦边框；列表选中项蓝色高亮；按钮按功能彩色淡底区分（蓝 = 新建 / 添加 / 编辑，红 = 删除 / 清除，绿 = 渲染 / 应用），鼠标悬停有变色反馈；状态提示颜色全软件统一（绿 = 成功、红 = 出错、灰 = 等待），整体清爽不花哨。

> 本项目为界面改良 + 功能扩展；核心计算在保持原版框架的基础上修正了概率抹零问题并补齐 XOR / NOT / VOTER 门（见第 8 点），遵循 BSD-2-Clause 协议。
...
- **macOS 风格界面**：主窗口与全部对话框统一白底扁平设计，按钮按功能彩色区分，状态提示颜色统一
- **交互式树编辑器**：实时图形预览，所见即所得
- **双分析模式**：FTA（自下而上的故障树分析）与 ETA（自上而下的事件树分析）
- **AI 智能助手**：内置对话式界面，支持故障树分析与改进建议
- **精确概率计算**：支持与门（AND）/ 或门（OR）/ 异或门（XOR）/ 非门（NOT）/ 表决门（VOTER，可设 k/n 阈值），全精度计算不抹零
- **可视化图形生成**：基于 Graphviz，逻辑门直接显示在节点中；默认竖向布局并自动适配中文字体与中文标签
- **多种导出格式**：JSON、XML、Excel（带层级结构）、Mermaid 流程图文本
- **零概率节点高亮**：快速定位问题节点
- **界面流畅不卡顿**：图形渲染与 AI 请求在后台线程执行，操作不阻塞
- **安全凭据存储**：API 密钥仅保存在本地并做混淆处理，绝不入库；支持环境变量 `FTA_AI_API_KEY`（优先级更高）
...
- **中文渲染标签**：节点内的概率、计算概率、日期、逻辑门（与门 / 或门 / 异或门 / 非门 / 表决门）等标签均为中文；新建分析时根节点默认为"根事件"、标题默认为"未命名分析"。
...
│   └── mermaid_exporter.py      # Mermaid 流程图文本导出
├── tests/                        # 测试套件（含 run_all_tests.py 一键运行）
├── data/examples/               # 示例数据
├── docs/                        # 文档
...
覆盖核心逻辑、ETA 计算、Excel 导出、链接计算、概率计算（含 XOR / NOT / VOTER 用例，共 18 个）、AI 更新流程与结构校验器。
...
- 本中文版在原版基础上将界面与文档调整为简体中文，新增国内 / 本地 AI 服务商支持，并修正了原版的概率抹零问题、补齐 XOR / NOT / VOTER 逻辑门
